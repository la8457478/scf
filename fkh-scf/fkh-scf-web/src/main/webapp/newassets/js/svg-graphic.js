...
(function(d3){
    ...
    d3.CreatLine.prototype.figureYunit = function(yMax){//计算实际点在图中对应的y坐标
        ...
        this.yDataUnit = Math.floor(yDataMax/2);
        this.yFigureUnits = (yMax-100)/yDataMax;
        return {
            yDataUnit: Math.floor(yDataMax/2),
            yFigureUnits:(yMax - 100)/yDataMax
        }
    }
    ...
    d3.CreatLine.prototype.yAixDrew = function(yMax){
        ...
        var oLine = creatTage("line",{x1:20,y1:0,x2:40,y2:yMax-30,stroke:"#c0d0e0"});
        var xEnd = this.settings.yLineHide ? this.xMax-20:this.xMax;
        var dasharray = '';
        ...
        for(var i=0;i<=2;i++){
            if(i == 0){
                ...
                var oLine1 = creatTage("line",{x1:20,y1:yMax-yUnit.yDataUnit*i*yUnit.yFigureUnits -30.5,x2:xEnd,y2:yMax-yUnit.yDataUnit*i*yUnit.yFigureUnits -30.5,stroke:"white","stroke-width":1,"stroke-dasharray":"5,5"});
            }else{
                var oLine1 = creatTage("line",{x1:20,y1:yMax-yUnit.yDataUnit*i*yUnit.yFigureUnits -30.5,x2:xEnd,y2:yMax-yUnit.yDataUnit*i*yUnit.yFigureUnits -30.5,stroke:"white"});
            }
    ...
    d3.CreatLine.prototype.xAixDrew = function(xMax){
        ...
        var oG = creatTage("g",{});
        if(this.settings.yLineHide == false){
            var oLine = creatTage("line",{x1:"20",y1:this.yMax-30,x2:this.xMax,y2:this.yMax-30,stroke:"#c0d0e0"});
        }else{
            var oLine = creatTage("line",{x1:"20",y1:this.yMax-30,x2:this.xMax-20,y2:this.yMax-30,stroke:"white"});
        }
        oG.appendChild(oLine);
        ...
        var oText =  creatTage("text",{x:this.xMax-80,y:25,"font-size":"20",'text-anchor':'middle',"fill":this.settings.textColor});
    ...
    d3.CreatLine.prototype.polyLineDrew = function(){
        ...
        if(piontsNum == ""){
            ...
            oCircle = creatTage('circle',{cursor:"move",dataX:this.settings.data[0].x,dataY:this.settings.data[0].y,cx:40,cy:(this.yMax-this.settings.data[0].y*this.yFigureUnits - 30),r:"2",'fill':'white','stroke':'86898c','stroke-width':'1'});
        ...
        for(var i=1;i<this.settings.data.length;i++){
            ...
            oCircle = creatTage('circle',{cursor:"move",dataX:this.settings.data[i].x,dataY:this.settings.data[i].y,cx:(i*this.xDataUnit+40),cy:(this.yMax-this.settings.data[i].y*this.yFigureUnits - 30),r:"2",'fill':'white','white':'#86898c','stroke-width':'2'});
    ...
    d3.CreatLine.prototype.customTextDrew = function(){
        if(!!this.settings.customText && this.settings.customText.length<1){
            return false;
        }
        var oG = creatTage("g",{});
        var xzhi = 0;
        for(var i=0;i<this.settings.customText.length;i+=1){
            xzhi = this.settings.customText[i].xMax == true ? this.xMax-80+this.settings.customText[i].x : this.settings.customText[i].x;
            var oText =  creatTage("text",{x:xzhi,y:this.settings.customText[i].y,"font-size":this.settings.customText[i].fontSize,"fill":this.settings.textColor});
            oText.textContent = this.settings.customText[i].text;
            oG.appendChild(oText);
        }
        this.oSvg .appendChild(oG);
    }
