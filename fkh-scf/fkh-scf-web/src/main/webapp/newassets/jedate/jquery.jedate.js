...
;(function(root, factory) {
    ...
    jedfn.initdates = function(){
        var that = this, opts = that.opts, newDate = new Date(),
            ...
            zIndex = opts.zIndex == undefined ? 100000000 : opts.zIndex,
    ...
    jedfn.clickDays = function (opts,boxCell) {
        ...
        if(opts.festival && lang.name == "cn") {
            ...
            tdCls.on("mouseover", function () {
                ...
                tipDiv.css({"z-index":  (opts.zIndex == undefined ? 100000000 + 5 : opts.zIndex + 5),top:tipPos.top,left:tipPos.left,position:"absolute",display:"block"});
                ...
                var hmsVal = that.gethmsVal(boxCell),
                    dateVal = function () {
                        var ymdObj = {}, ymday = (isYYMM || isYY) ? ".ymcon":".daysul";
                            //修改时间控件选择年月日不选择日期bug
                            if(boxCell.find(ymday).find("td.actdate").attr("je-val") == undefined){
                                return false;
                            }
